...
class AgentNetwork(object):
    ...
    def create_agent_network(self, scope, state_size):
        with tf.name_scope(scope):
            ...
            #MLP_output = tf.clip_by_value(MLP_output, 1e-5, 1 - 1e-5)
    ...
    def get_network_parameter(self):
        Weights_for_MLP = tf.get_default_graph().get_tensor_by_name("Activate/high/Weights_for_MLP:0")
        Weights_for_MLP_get = self.sess.run(Weights_for_MLP)
        Bias_for_MLP = tf.get_default_graph().get_tensor_by_name("Activate/high/Bias_for_MLP:0")
        Bias_for_MLP_get = self.sess.run(Bias_for_MLP)
        H_for_MLP = tf.get_default_graph().get_tensor_by_name("Activate/high/H_for_MLP:0")
        H_for_MLP_get = self.sess.run(H_for_MLP)

        return Weights_for_MLP_get, Bias_for_MLP_get, H_for_MLP_get
